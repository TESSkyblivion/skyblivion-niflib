...
const Type & RefObject::GetInternalType() const {
	return TYPE;
}
...
bool RefObject::IsSameType( const Type & compare_to) const {
	return GetInternalType().IsSameType( compare_to );
}

bool RefObject::IsSameType( const RefObject * object ) const {
	return GetInternalType().IsSameType( object->GetInternalType() );
}

bool RefObject::IsDerivedType( const Type & compare_to) const {
	return GetInternalType().IsDerivedType( compare_to );
}

bool RefObject::IsDerivedType( const RefObject * object ) const {
	return GetInternalType().IsDerivedType( object->GetInternalType() );
}
...
string RefObject::GetIDString() const {
	stringstream out;
	out << this << "(" << this->GetInternalType().GetTypeName() << ")";
	return out.str();
}