...
#include <array>
#if (_MSC_VER < 1600)
#define __std__array std::tr1::array
#else
#define __std__array std::array
#endif
...
void ComplexShape::Merge(NiAVObject* root)
{
	...
	//				map<__std__array<int, 3>, int> faces_indices;

	//				for (int z = 0; z < faces.size(); z++)
	//				{
	//					__std__array<int, 3> current_face_indices;
	...
	//				{
	//					__std__array<int, 3> current_combination;
	//					__std__array<int, 3> current_face_indices;
	...
	for (unsigned int shape_num = 0; shape_num < shapes.size(); ++shape_num)
	{
		...
		if (propGroups.size() > 0)
		{
			...
			for (vector<NiPropertyRef>::const_iterator prop = propGroups[shape_num].begin(); prop != propGroups[shape_num].end(); ++prop)
			{
				...
				if (current_property->GetInternalType().IsSameType(BSLightingShaderProperty::TYPE))
				{
			...
			else
			{
				...
				for (vector<NiPropertyRef>::const_iterator prop = propGroups[shape_num].begin(); prop != propGroups[shape_num].end(); ++prop)
				{
					if ((*prop)->GetInternalType().IsSameType(NiAlphaProperty::TYPE))
					{
