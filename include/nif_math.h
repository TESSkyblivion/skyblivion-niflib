...
struct Triangle : public Compound {
	...
	NIFLIB_API bool operator==(const Triangle &other) const
	{
		return
...
struct Matrix22 : public Compound {
	...
	NIFLIB_API bool operator==(const Matrix22 & m) const {
		return (
			m.rows[0][0] == rows[0][0] && m.rows[0][1] == rows[0][1] && 
...
struct Matrix34 : public Compound {
	...
	/*! Copy constructor.  Initializes Matrix to another Matrix34.
	* \param[in] m The matrix to initialize this one to.
	*/
	NIFLIB_API Matrix34(const Matrix34 & m) { memcpy(rows, m.rows, sizeof(Float4) * 4); }
	...
	*/
	NIFLIB_API friend ostream & operator<<(ostream & lh, const Matrix34 & rh);

	NIFLIB_API bool operator==(const Matrix34 & m) const {
		return (
			m.rows[0][0] == rows[0][0] && m.rows[0][1] == rows[0][1] && m.rows[0][2] == rows[0][2] && m.rows[0][3] == rows[0][3] &&
			m.rows[1][0] == rows[1][0] && m.rows[1][1] == rows[1][1] && m.rows[1][2] == rows[1][2] && m.rows[1][3] == rows[1][3] &&
...
struct Color3 : public Compound {
	...
	NIFLIB_API bool operator==(const Color3 & other) const {
		return (
			r == other.r &&
			g == other.g &&
			b == other.b
